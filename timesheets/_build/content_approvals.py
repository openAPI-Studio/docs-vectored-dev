...
APPROVALS = [
    ("approvals", dict(
        ...
        blocks=[
            ...
            P("The top of the page counts what is outstanding: how many items, across how many people, and how many hours. If anything has been waiting three days or more, it says how long the oldest has sat there — a queue that is merely long is different from one that is being neglected."),
            SHOT("timesheets-approvals-queue.png", "The Approvals tab showing the waiting-on-you count, the filter bar, and items grouped by person"),

            H("Grouped by person"),
            P("Items are grouped under the person who submitted them, oldest group first — whoever has waited longest is at the top, however much or little they sent."),
            P("Each group header carries that person's item count, their total hours, and the age of their oldest item. **Approve all** decides the whole group in one action."),
            P("That is the point of the grouping: approving a colleague's week is usually **one** judgement, and it should not have to be expressed as six identical clicks."),
            P("You can group **by type** instead — time, leave, weeks — or turn grouping off for a single flat list. Sorting is oldest-first by default, and can be reversed."),

            H("Reading a row"),
            P("Every item is one row: how much, what it is for, and how long it has been waiting."),
            ...
            NOTE("**Three days is a display threshold, not policy.** It is roughly when a wait becomes worth noticing. It is deliberately not tied to the auto-approve timeout, which is a separate decision and may be switched off entirely."),
            P("A submitted week expands in place to show the entries inside it. **History** on any row opens its full decision trail."),
            ...
            WARN("**Leave reasons are not searchable, and that is deliberate.** A reason can describe a medical condition. Making it a search term would mean typing a condition into a filter box and getting back a list of the people who have it. The reason is not sent with the queue at all — it is fetched only when you press **Show reason** on that one request."),
            P("Tick rows and use the bulk bar to approve or send back several at once, up to **100 items** in one action. The bar appears at the bottom of the screen as soon as anything is selected, and says how many items and how many hours it is about to commit."),
            P("Selection is scoped to what is on screen: applying a filter that hides a selected row also unselects it, so nothing gets decided that you could not see. Refreshing clears the selection for the same reason."),
            P("A rejection comment is validated once for the whole batch rather than per item, so a bulk send-back with no comment is refused outright instead of half-applying."),
            P("If some items in a batch cannot be decided — usually because another approver got there first — the queue reports exactly which ones failed and why, rather than reporting success for the whole batch."),
            SHOT("timesheets-approvals-multiselect.png", "The Approvals queue with several rows selected and the dark bulk action bar pinned at the bottom of the screen"),

            H("Rejecting, and approving with a note"),
            P("Sending something back **requires** a comment. The person is told what to fix, they are emailed it, and it is kept in the audit trail."),
            P("An approval can carry a note too — the message icon beside the decision buttons, on a row or on the bulk bar. That is optional: *approved, but split this by project next time* is worth saying, and requiring a comment on every approval would only produce a hundred of them reading \"ok\"."),
            ...
            H("The side panel"),
            P("Beside the queue sits the context that is not about any one row."),
            TABLE(["Panel", "What it tells you"], [
                ...
                ["Covering for", "Whose approvals you are standing in for, and until when. See [Delegation](delegation.html)"],
            ]),
            P("Decisions you make while covering for somebody are recorded under **your** name in the approval history, with the delegation noted."),
            SHOT("timesheets-approvals-rail.png", "The Approvals side panel showing the waiting-time breakdown, a project with no approvers, and an active delegation"),
